...
import type {
  BackendCourseDetail,
  CourseExercise,
} from '@/lib/courses/backendCourseDetails'
...
function BackendCoursePage() {
  ...
  const courseIndex = useMemo(() => {
    ...
    const match = backendCourses.find((item) => item.id === course.id)
  ...
  const courseCompleted = course ? backendProgress[course.id] : false
  ...
  useEffect(() => {
    ...
    const loadCourse = async () => {
      ...
      try {
        ...
        if (isMounted) {
          if (data) {
            setCourse(data as BackendCourseDetail)
          } else {
            setCourse(getBackendCourseDetail(courseId) ?? null)
          }
        }
      } catch (error) {
      ...
      <div className={`${geistSans.variable} min-h-screen bg-gray-50 font-sans`}>
        ...
        <main className="mx-auto flex max-w-4xl flex-col gap-6 px-4 py-16 sm:px-6 lg:px-8">
          <Card hover={false}>
            ...
            <p className="mt-2 text-gray-600">
              We could not find that course. Head back to the Backend &amp; Systems
...
export default withAuth(BackendCoursePage)
